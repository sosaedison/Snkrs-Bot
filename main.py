...
def run(
    ...
    while True:
        try:
            ...
            (
                skip_add_address,
                skip_select_shipping,
                skip_payment,
            ) = poll_checkout_phase_one(driver=driver)
...
def check_add_new_address_button(driver):
    ...
    LOGGER.info("Waiting for Add New Address button to become clickable")
    wait_until_clickable(driver, xpath=xpath, duration=0.2)
...
def check_shipping(driver):
    ...
    LOGGER.info("Waiting for shipping options become visible")
    wait_until_visible(driver, xpath=xpath, duration=0.2)
...
def poll_checkout_phase_one(driver):
    ...
    while True:
        ...
        if checkout_num_retries_attempted < checkout_num_retries:
            ...
            LOGGER.info(
                "Too many iterations through checkout element check. Terminating check..."
            )
            break
...
def poll_checkout_phase_two(driver):
    ...
    while True:
        ...
        if checkout_num_retries_attempted < checkout_num_retries:
            ...
            LOGGER.info(
                "Too many iterations through checkout element check #2. Terminating check..."
            )
            break
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--shoe-type",
        default="M",
        choices=("M", "W", "Y", "C", "XXS", "XS", "S", "L", "XL"),
    )
    parser.add_argument(
        "--shipping-option",
        default="STANDARD",
        choices=("STANDARD", "TWO_DAY", "NEXT_DAY"),
    )
    ...
    run(
        ...
        select_payment=args.select_payment,
        purchase=args.purchase,
        num_retries=args.num_retries,
        dont_quit=args.dont_quit,
        cvv=args.cvv,
    )
